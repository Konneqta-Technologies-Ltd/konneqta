...
export async function resolveActiveCardRedirect(): Promise<ActiveCardResolution> {
  ...
  // A profile without a usable card is incomplete. Sending the user to their
  // username would land on a missing public card and can create a redirect
  // loop, so return them to the repairable setup form instead.
  return { status: 'onboard' };
}
