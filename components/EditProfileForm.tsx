    ...
    try {
      ...
      if (cardError) {
        ...
        if (
          ...
          errorMessage =
            "There's an issue with your username. Please ensure it's 3-20 characters and contains only a-z, 0-9, or _.";
        }
      ...
      if (linksToInsert.length > 0) {
        ...
        if (linksError) {
          toast.error(
            "Profile updated, but we couldn't save your social links.",
          );
          return;
        }
      }
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
          ...
          <div>
            ...
            >
              ...
              <span className="text-xs font-normal text-zinc-400 dark:text-zinc-500">
                (locked — this is your account identity)
              </span>
            </label>
            <input
              id="username"
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${form.show_phone ? 'translate-x-4' : 'translate-x-0.5'}`}
                />
              </button>
              ...
              <InfoTip
                content="When ON, your phone number is shown publicly on your card and included in the .vcf contact file people download. When OFF, it stays private."
                side="top"
              />
            </div>
          </div>
          ...
          <div>
            ...
            />
            <p
              className={`mt-1 text-right text-xs ${form.bio.length > BIO_MAX_CHARS ? 'text-red-500' : 'text-zinc-400 dark:text-zinc-500'}`}
            >
            ...
            {hasLinkLimit && socialLinks.length >= maxSocialLinks && (
              <div className="mb-2 flex items-center justify-between gap-2 rounded-lg border border-(--main-orange)/30 bg-(--main-orange)/5 px-3 py-2 text-xs">
                <span className="text-zinc-600 dark:text-zinc-400">
                  Limit reached ({maxSocialLinks} links). Upgrade to Pro for up
          ...
          <div className="mt-4 flex gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex flex-1 items-center justify-center gap-2 cursor-pointer rounded-lg bg-(--main-orange) px-4 py-3 text-sm font-medium text-white transition-colors hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
            >
          ...
          >
            ...
            <div className="mt-6 flex gap-3">
              <button
                type="button"
                onClick={() => setShowCancelDialog(false)}
                className="flex-1 cursor-pointer rounded-lg bg-zinc-200 px-4 py-3 text-sm font-medium text-zinc-700 hover:bg-zinc-300 dark:bg-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-600"
              >
                Cancel
              </button>
              <button
                type="button"
                onClick={() => router.push(`/${cardSlug}`)}
                className="flex-1 cursor-pointer rounded-lg bg-red-500 px-4 py-3 text-sm font-medium text-white hover:bg-red-600"
              >
                Discard
              </button>
            </div>
